...
INSTRUCTION_HANDLER(AssertionArrayPrefix) {
  ...
  assert(instruction.direct_children_count > 0);
  result = target.empty();
  const auto prefixes{
      static_cast<std::size_t>(instruction.direct_children_count) - 1};
  ...
  if (!result) [[likely]] {
    ...
    std::size_t entry_offset{children_start_};
    ...
    for (std::size_t child_index = entry_offset + 1;
...
INSTRUCTION_HANDLER(LogicalOr) {
  ...
  result = instruction.children_count == 0;
  const auto &target{get(instance, instruction.relative_instance_location)};
...
INSTRUCTION_HANDLER(LogicalCondition) {
  ...
  const auto children_size{
      static_cast<std::size_t>(instruction.direct_children_count)};
  ...
  // Walk condition children [0..condition_end)
  std::size_t condition_flat{children_start_};
  for (std::size_t cursor = 0; cursor < condition_end; cursor++) {
    if (!EVALUATE_RECURSE(schema.instructions[condition_flat], target)) {
      result = false;
      break;
    }
  ...
  if (consequence_start > 0) {
    ...
    std::size_t consequence_flat{children_start_};
...
INSTRUCTION_HANDLER(ControlGroupWhenDefines) {
  ...
  assert(instruction.children_count > 0);
...
INSTRUCTION_HANDLER(LoopPropertiesMatch) {
  ...
  for (const auto &entry : target.as_object()) {
    ...
    for (std::size_t child_index = *child_offset + 1;
