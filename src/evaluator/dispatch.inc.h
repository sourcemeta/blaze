...
INSTRUCTION_HANDLER(AssertionDefines) {
  ...
  const auto &value{instruction.value.property};
...
INSTRUCTION_HANDLER(AssertionDefinesAll) {
  ...
  const auto &value{instruction.value.string_set};
...
INSTRUCTION_HANDLER(AssertionPropertyDependencies) {
  ...
  const auto &value{instruction.value.string_map};
  // Otherwise we are we even emitting this instruction?
  assert(!value.empty());
  result = true;
  const auto &object{target.as_object()};
...
INSTRUCTION_HANDLER(AssertionType) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto value{instruction.value.type};
...
INSTRUCTION_HANDLER(AssertionTypeAny) {
  ...
  const auto &value{instruction.value.types};
  // Otherwise we are we even emitting this instruction?
  assert(value.size() > 1);
  const auto &target{get(instance, instruction.relative_instance_location)};
  // In non-strict mode, we consider a real number that represents an
...
INSTRUCTION_HANDLER(AssertionTypeStringBounded) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto &value{instruction.value.range};
...
INSTRUCTION_HANDLER(AssertionTypeStringUpper) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto value{instruction.value.unsigned_integer};
...
INSTRUCTION_HANDLER(AssertionRegex) {
  ...
  const auto &value{instruction.value.regex};
...
INSTRUCTION_HANDLER(AssertionEqual) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto &value{instruction.value.json};
  result = (target == value);
...
INSTRUCTION_HANDLER(AssertionEqualsAny) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto &value{instruction.value.set};
...
INSTRUCTION_HANDLER(AssertionStringType) {
  ...
  const auto value{instruction.value.string_type};
  switch (value) {
...
INSTRUCTION_HANDLER(LogicalOr) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto value{instruction.value.boolean};
...
INSTRUCTION_HANDLER(LogicalCondition) {
  ...
  result = true;
  const auto value{instruction.value.index_pair};
...
INSTRUCTION_HANDLER(ControlEvaluate) {
  ...
  if (callback.has_value()) {
    const auto &value{instruction.value.pointer};
...
INSTRUCTION_HANDLER(ControlDynamicAnchorJump) {
  ...
  result = false;
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto &value{instruction.value.string};
...
INSTRUCTION_HANDLER(LoopPropertiesUnevaluatedExcept) {
  ...
  EVALUATE_BEGIN_NON_STRING(LoopPropertiesUnevaluatedExcept,
                            target.is_object());
  assert(!instruction.children.empty());
  const auto &value{instruction.value.property_filter};
  result = true;
...
INSTRUCTION_HANDLER(LoopPropertiesMatch) {
  ...
  const auto &value{instruction.value.named_indexes};
  assert(!value.empty());
  result = true;
  for (const auto &entry : target.as_object()) {
...
INSTRUCTION_HANDLER(LoopPropertiesExactlyTypeStrict) {
  ...
  const auto &target{get(instance, instruction.relative_instance_location)};
  const auto &value{instruction.value.typed_properties};
  if (!target.is_object()) {
